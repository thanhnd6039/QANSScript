...
//        headerCols[1] = "MAIN SALES REP";
        ...
        for (int rowIndex = 0; rowIndex < dataOfRevCostDump.size(); rowIndex++){
            ...
            if (parentClassCol.equalsIgnoreCase("COMPONENTS") ||
                ...
                if (yearCol.equalsIgnoreCase(year) && quarCol.equalsIgnoreCase(strQuar)){
                    String oemGroupCol = dataOfRevCostDump.get(rowIndex)[1].toString().trim();
                    if (oemGroupCol.contains(":")){
                        String[] strArr = oemGroupCol.split(":", 2);
                        oemGroupCol = strArr[1].toString().trim();
                    }
                    String pnCol = dataOfRevCostDump.get(rowIndex)[10].toString().trim();
                    String rQtyCol = dataOfRevCostDump.get(rowIndex)[27].toString().trim();
                    String rAmountCol = dataOfRevCostDump.get(rowIndex)[28].toString().trim();
                    String bQtyCol = dataOfRevCostDump.get(rowIndex)[32].toString().trim();
                    String bAmountCol = dataOfRevCostDump.get(rowIndex)[34].toString().trim();
                    String bfQtyCol = dataOfRevCostDump.get(rowIndex)[35].toString().trim();
                    String bfAmountCol = dataOfRevCostDump.get(rowIndex)[37].toString().trim();
                    String cfQtyCol = dataOfRevCostDump.get(rowIndex)[38].toString().trim();
                    String cfAmountCol = dataOfRevCostDump.get(rowIndex)[40].toString().trim();
                    Object[] cols = new Object[10];
                    cols[0] = oemGroupCol;
                    cols[1] = pnCol;
                    cols[2] = rQtyCol;
    ...
    public void verifySGReport(){
        ...
        List<String> listOfOEMGroupFromSGReport = getListOfOEMGroupFromSGReport();
        List<String> listOfOEMGroupFromRevCostDump = getListOfOEMGroupFromRevCostDump();

    }
    public List<String> getListOfOEMGroupFromSGReport(){
        List<String> listOfOEMGroup = new ArrayList<>();
        String targetFilePath = "C:\\CucumberFramework\\Downloads\\Target.xlsx";
        List<Object[]> dataOfSGReport = FileReaderManager.getInstance().getExcelReader().readDataFromExcel(targetFilePath, 0, 1, 0);
        for (int rowIndex = 0; rowIndex < dataOfSGReport.size(); rowIndex++){
            listOfOEMGroup.add(dataOfSGReport.get(rowIndex)[0].toString().trim());
        }
        listOfOEMGroup = SetUniqueList.setUniqueList(listOfOEMGroup);
        return listOfOEMGroup;
    }
    public List<String> getListOfOEMGroupFromRevCostDump(){
        List<String> listOfOEMGroup = new ArrayList<>();
        String sourceFilePath = "C:\\CucumberFramework\\Downloads\\Source.xlsx";
        List<Object[]> dataOfRevCostDump = FileReaderManager.getInstance().getExcelReader().readDataFromExcel(sourceFilePath, 0, 1, 0);
        for (int rowIndex = 0; rowIndex < dataOfRevCostDump.size(); rowIndex++){
            listOfOEMGroup.add(dataOfRevCostDump.get(rowIndex)[0].toString().trim());
        }
        listOfOEMGroup = SetUniqueList.setUniqueList(listOfOEMGroup);
        return listOfOEMGroup;
    }
